...
class robot:
    ...
    def measurement_prob(self, measurement):
        ...
        for i in range(len(landmarks)):
            dist = sqrt((self.x - landmarks[i][0]) ** 2 + (self.y - landmarks[i][1]) ** 2)
            gaussian_temp = self.Gaussian(dist, self.sense_noise, measurement[i])
            if gaussian_temp > 0.079:
	      print "gaussian greater than 0.79:", gaussian_temp, dist, self.sense_noise, measurement[i]
            prob *= gaussian_temp
        return prob
